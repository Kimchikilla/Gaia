...
\caption{Overview of the Gaia pipeline: data collection from four public sources, six-step preprocessing, two-step continual pre-training of MGM with vocabulary expansion (9,669 $\rightarrow$ 12,916 genera), and downstream evaluation across ten benchmark tasks.}
\label{fig:pipeline}
\end{figure}
...
\caption{Benchmark comparison between Gaia and Random Forest across all evaluation tasks. Gaia matches RF on classification tasks and outperforms it on temporal yield prediction.}
\label{fig:benchmark}
\end{figure}
...
\caption{Future yield prediction: Gaia ($R^2$=0.541) outperforms Random Forest ($R^2$=0.504) when predicting next year's yield from this year's microbiome composition. This is the only task where Gaia exceeds RF, highlighting the value of learned temporal representations.}
\label{fig:future}
\end{figure}
...
\caption{Performance scaling with training data quantity. All metrics improve consistently from 3,624 to 8,329 samples without saturation, indicating that additional soil samples will further enhance Gaia's capabilities.}
\label{fig:scaling}
\end{figure}
...
\subsection{Out-of-Distribution Generalization: Bernburg Long-Term Trial}

To test whether Gaia's learned representations transfer to a soil site never seen during pre-training, we acquired the Bernburg long-term field trial dataset (96 samples, 763 genera, 3 years 2019--2021) from the same author as Westerfeld but located in a different geographic and management context. Bernburg samples were not included in any of the 8,329 training samples. We evaluated Gaia under two regimes.

\textbf{Regime 1: Linear probe on Bernburg.} The Gaia v4 backbone was frozen and a task-specific head was trained on 80\% of Bernburg samples and tested on the held-out 20\%, identical to our other benchmarks. Random Forest used the same 80/20 split.

\begin{table}[H]
\centering
\caption{Bernburg linear-probe results: backbone frozen, head trained on 80\% Bernburg, tested on 20\%. Gaia outperforms RF on 5 of 6 tasks.}
\label{tab:bernburg_lp}
\begin{tabular}{lccc}
\toprule
\textbf{Task} & \textbf{Gaia} & \textbf{Random Forest} & \textbf{Winner} \\
\midrule
Tillage Classification & 84.2\% & 100.0\% & RF \\
Fertilization Classification & \textbf{68.4\%} & 57.9\% & \textbf{Gaia} \\
pH Prediction & \textbf{$R^2$=0.585} & 0.551 & \textbf{Gaia} \\
\textbf{Total Carbon ($C$ \%)} & \textbf{$R^2$=0.718} & 0.357 & \textbf{Gaia} \\
Total Nitrogen ($N$ \%) & \textbf{$R^2$=0.732} & 0.725 & \textbf{Gaia} \\
Organic Matter ($OM$ \%) & \textbf{$R^2$=0.717} & 0.497 & \textbf{Gaia} \\
\bottomrule
\end{tabular}
\end{table}

The most striking result is total carbon prediction: Gaia achieves $R^2$=0.718 versus Random Forest's $R^2$=0.357---approximately twice the explained variance. This pattern, where Gaia underperforms RF on the same site (Westerfeld carbon: 0.829 vs RF 0.948) but outperforms RF on a new site, suggests that the foundation model's learned representations are more robust to site-specific noise than feature-based methods.

\textbf{Regime 2: True zero-shot cross-site transfer.} To test whether Gaia generalizes without any exposure to the target site, we trained the head exclusively on Westerfeld and evaluated directly on Bernburg---no Bernburg data was used during head training. Random Forest baseline followed the same protocol.

\begin{table}[H]
\centering
\caption{Zero-shot cross-site transfer: head trained on Westerfeld only, evaluated on Bernburg. Gaia outperforms RF on all three regression tasks.}
\label{tab:bernburg_zs}
\begin{tabular}{lcc}
\toprule
\textbf{Task} & \textbf{Gaia} & \textbf{Random Forest} \\
\midrule
Total Carbon ($C$ \%) & \textbf{$R^2$=0.291} & 0.199 \\
pH & \textbf{$R^2$=0.393} & $-$0.515 \\
Total Nitrogen ($N$ \%) & \textbf{$R^2$=0.520} & 0.307 \\
\bottomrule
\end{tabular}
\end{table}

Gaia outperforms Random Forest on all three regression tasks under true zero-shot transfer. The pH result is particularly informative: Random Forest's negative $R^2$ ($-$0.515) indicates worse-than-mean prediction---the feature-based model fails completely when applied to a new site---while Gaia achieves a positive $R^2$ of 0.393. Although absolute $R^2$ values are necessarily lower than within-site benchmarks (the two sites differ in soil type, management history, and climate), the consistent advantage of Gaia over RF demonstrates that learned microbial community representations transfer across sites in a way that engineered features do not.

\textbf{Implication:} This is the strongest evidence in our study that soil-specialized foundation models capture transferable ecological structure. Random Forest treats each genus as an independent feature with site-specific weights; when those weights are applied to a new site, they fail. Gaia's representations, learned through self-supervised next-token prediction across 8,329 heterogeneous samples, encode relationships between taxa that remain meaningful in unseen contexts.
